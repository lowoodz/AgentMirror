...
mac_installed_app_test() {
  ...
  for _ in $(seq 1 90); do
    if smr_curl_health_ok "http://127.0.0.1:8080"; then
      ok=1
      break
    fi
  ...
  smr_curl_ui_ok "http://127.0.0.1:8080"
  ...
  SMR_ATTACH=1 SMR_BASE=http://127.0.0.1:8080 SMR_CONFIG="$config_path" python3 scripts/blackbox_test.py
}
