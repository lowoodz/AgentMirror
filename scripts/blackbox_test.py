#!/usr/bin/env python3
"""Black-box tests simulating real SafeRoute usage scenarios."""
...
FILE_SECRET = "UNIQUEBLACKBOXFILESECRETXYZ998877" + "X" * 32
OTHER_FILE_SECRET = "ORPHANWIDGETQUANTUMZULU776655" + "X" * 36
PARENT_ONLY_SECRET = "PARENTTOPREPORTSECRETALPHA" + "X" * 39
CHILD_ONLY_SECRET = "CHILDSUBREPORTSECRETBETA" + "X" * 41
...
def apply_test_config(glm: str, ds: str, secrets_dir: Path, mock_ports: dict[str, int]) -> bool:
    ...
    config_path = __import__("os").environ.get("SMR_CONFIG", "").strip()
    if ATTACH and config_path:
        path = Path(config_path)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(build_config(glm, ds, listen, secrets_dir, mock_ports), encoding="utf-8")
        if reload_config(BASE) == 200:
            time.sleep(1.5)
...
def run_all_scenarios(report: Report, secrets_dir: Path) -> None:
    ...
    scenario_tier_path_routing(report)
...
def scenario_openai_python_sdk(report: Report) -> None:
    ...
    report.add(story, "openai_python_sdk", False, last_err, (time.perf_counter() - start) * 1000)
...
def scenario_session_window_exhaustion(report: Report, secrets_dir: Path) -> None:
    ...
    for attempt in range(3):
        ...
        if code_tr == 200:
            wait_for_session_audit(BASE, session, after_ids=known_ids, timeout=20.0)
            break
        trigger_detail = text_tr[:120].replace("\n", " ")
        time.sleep(2.0 * (attempt + 1))
    wait_server_idle(BASE, timeout=30.0)
    dlp_counts: list[int] = []
    for i in range(3):
    ...
    detail = f"trigger_status={code_tr}, dlp_per_turn={dlp_counts}"
    if not ok and trigger_detail:
        detail += f", trigger_err={trigger_detail!r}"
    report.add(
        story,
        "session_window_exhaustion",
        ok,
        detail,
        0,
    )
...
def scenario_stream_fallback_no_token(report: Report) -> None:
    ...
    session = "blackbox-stream-fallback"
    wait_server_idle(BASE, timeout=60.0)
    code, raw, ms = 0, "", 0.0
    chain: list = []
    has_token = False
    for attempt in range(3):
        ...
        audit = latest_audit_for_session(BASE, session)
        chain = audit.get("fallback_chain", []) if audit else []
        has_token = "content" in raw or "delta" in raw
        if code == 200 and len(chain) >= 2 and has_token:
            break
        if attempt < 2:
            time.sleep(2.0 * (attempt + 1))
...
def scenario_silent_fallback(report: Report) -> None:
    ...
    session = "blackbox-transparent-fallback"
    wait_server_idle(BASE, timeout=60.0)
    code, _, ms = 0, "", 0.0
    chain: list = []
    for attempt in range(3):
...
def scenario_tier_path_routing(report: Report) -> None:
    ...
    body = {
        "model": "mock-model",
        "messages": [{"role": "user", "content": "Reply exactly: tier-ok"}],
        "max_tokens": 16,
    }
    code_m, text_m, ms_m = http(
        "POST",
        f"{BASE}/medium/v1/chat/completions",
        body=body,
        timeout=30.0,
    )
    ok_m = code_m == 200 and "choices" in text_m
    report.add(story, "tier_path_medium", ok_m, f"status={code_m}", ms_m)

    code_l, text_l, ms_l = http(
        "POST",
        f"{BASE}/lite/v1/chat/completions",
        body=body,
        timeout=30.0,
    )
    ok_l = code_l == 200 and "choices" in text_l
    report.add(story, "tier_path_lite", ok_l, f"status={code_l}", ms_l)
...
def scenario_admin_dashboard(report: Report) -> None:
    ...
    status_obj = json.loads(status) if c2 == 200 else {}
    ok = (
        c1 == 200
        and "LLM-SafeRoute" in ui
        and c2 == 200
        and status_obj.get("proxy_url_high")
        and status_obj.get("proxy_url_medium")
        and status_obj.get("proxy_url_lite")
        and c3 == 200
        and c4 == 200
