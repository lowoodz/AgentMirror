...
def main() -> int:
    ...
    vault = secrets / "vault"
    vault.mkdir()
    (secrets / "probe.txt").write_text(FILE_PROBE_SECRET, encoding="utf-8")
    ...
    try:
        ...
        subprocess.run(
            ["bash", "-lc", f"lsof -ti :{PORT} | xargs kill -9 2>/dev/null || true"],
            check=False,
        )
        ...
        results.append(check("web_ui", code == 200 and "LLM-SafeRoute" in ui, f"bytes={len(ui)}"))
        ...
        file_session = "install-func-file-session"
        http(
            "POST",
            f"{BASE}/v1/chat/completions",
            body={
            ...
            headers={"X-SMR-Session-Id": file_session},
        )

        chat_result: dict[str, object] = {}

        def do_leak_chat() -> None:
            code, text, ms = http(
                "POST",
                f"{BASE}/v1/chat/completions",
                body={
        ...
        dlp = dlp_after_chat(BASE, file_session, do_leak_chat)
        code = int(chat_result.get("code", 0))
        ms = float(chat_result.get("ms", 0.0))
        results.append(
