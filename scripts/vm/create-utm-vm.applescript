-- Create SecureModelRoute Windows x86_64 test VM in UTM (Apple Silicon: QEMU emulation).
-- Env: SMR_VM_NAME (default SecureModelRoute-Win11-x64), SMR_VM_ISO (required path)

on run
	set vmName to system attribute "SMR_VM_NAME"
	if vmName is missing value or vmName is "" then set vmName to "SecureModelRoute-Win11-x64"
	...
	tell application "UTM"
		...
		set newVM to make new virtual machine with properties {backend:qemu, configuration:{¬
			name:vmName, ¬
			notes:"SecureModelRoute Windows x86_64 test VM (OpenSSH / windows_vm_test.sh)", ¬
			architecture:"x86_64", ¬
			memory:8192, ¬
			cpu cores:4, ¬
			hypervisor:false, ¬
