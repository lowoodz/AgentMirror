...
if [[ "${SMR_SKIP_VM_TESTS:-0}" != "1" ]] && [[ -x "$UTMCTL" ]]; then
  if "$UTMCTL" list 2>/dev/null | grep -q started; then
    ...
    echo ">>> SKIP VM tests: no started UTM guest" | tee -a "$SUMMARY"
  fi
else
  echo ">>> SKIP VM tests" | tee -a "$SUMMARY"
fi
