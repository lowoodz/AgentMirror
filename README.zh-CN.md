...
- 从 LLM 代理流量反向重建智能体的目标、决策与行动，提供因果轨迹图、行动反思报告、智能体日报，解决多智能体、长程运行时代Agent的可观测性问题，并协助提升Agent Orchestration Loop（外层编排循环）的可靠性和任务成功率；
- 支持 macOS、Windows 桌面应用，一键安装。
...
| **macOS**（Intel）         | `AgentMirror_*_x86_64.dmg`       | 同 arm64 DMG（在 Apple Silicon 主机上交叉构建）                              |
| **macOS**（Intel）         | `smr-*-darwin-x86_64-app.tar.gz` | 解压后将 `AgentMirror.app` 放入 `/Applications`                         |
| **macOS**（Intel）         | `smr-*-darwin-x86_64.tar.gz`     | 仅 CLI                                                             |
...
- **因果轨迹** — 每次 Run 的 Goal → Decision → Action → Observation；支持有向图、思维导图、时间线视图
- **行动反思报告** — **默认 LLM 优先**（`insight.llm_critic: true`）：Run 进行中每 10 轮触发一次，Run 结束或空闲 10 分钟后也会生成；五维审视并跟踪原始/当前目标。仅在上游 LLM 不可用时回退规则基线（设 `llm_critic: false` 可恢复纯规则模式）
- **智能体日报** — **默认 LLM 优先**（`insight.llm_daily: true`），基于各 Run 反思报告汇总任务、问题与建议；界面会显示「生成中 / 无运行 / 已是最新」等状态
- **多智能体** — 按 `X-SMR-Agent-Id`、system prompt 指纹或空闲/Run 边界分离
- **智能体画像与行动模式** — 由 tools/统计推断能力画像；反思报告中展示行动模式挖掘结果
- **Run 管理** — 合并/拆分 Run；Run 卡片上交叉高亮 DLP 与操作安全风险
- **日报导出** — 浏览器打印 / HTML 视图（`GET /api/insight/daily/{date}/print`）

请先在 **日志** 页启用流量快照（`insight.require_traffic_bodies` 会自动开启 `logging.save_traffic_bodies`）。被拦截/失败的代理请求默认不进入 Insight；设 `insight.include_failed_requests: true` 可追踪（仅请求体）。

**保留策略：** Insight 数据默认 **30 天**（`insight.retention_days`）；流量快照默认 **7 天**（`logging.traffic_retention_days`）。若需从快照重放长会话，请增大 traffic 保留天数，并通过 `POST /api/insight/reset` 且 `replay_from_traffic: true` 重建。

完整 API 见 [AgentMirror 设计文档](docs/AgentMirror-Design.md)。
...
insight:
  ...
  llm_critic: true              # 默认 LLM-only 反思报告
  llm_daily: true               # 默认 LLM-only 日报
...
logging:
  ...
  traffic_retention_days: 7       # Insight 从快照重放需在此窗口内
...
| `GET /api/insight/agents`、`/runs`、`/runs/{id}/graph`、`/runs/{id}/report` | AgentMirror 运行、轨迹图、反思报告 |
| `GET /api/insight/daily/{date}`、`POST /api/insight/daily/generate`       | 日报                      |
| `GET /api/insight/agents/{id}/profile`                                   | 智能体能力画像                 |
| `POST /api/insight/reset`                                                | 清空 Insight 数据；可选从流量快照重放 |


完整 Insight API：[docs/AgentMirror-Design.md](docs/AgentMirror-Design.md#6-api)。
...
更多设计细节：[docs/AgentMirror-Design.md](docs/AgentMirror-Design.md)。
